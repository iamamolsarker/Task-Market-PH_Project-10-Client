...
const EditTask = () => {
  ...
  const { _id, title, category, description, deadline, budget, status } =
    useLoaderData();
  ...
  const handleEditTask = (e) => {
    ...
    fetch(
      `https://assignment-10-task-market-server.vercel.app/all-task/${_id}`,
      {
        method: "PUT",
        headers: {
  ...
  return (
    <div className="w-xl mx-auto py-10">
      ...
      <form onSubmit={handleEditTask} className="space-y-3">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-lg font-medium">User Email</label>
          <input
